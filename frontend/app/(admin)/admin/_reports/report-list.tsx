  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      const { data } = await apiClient.post(BASE, {
        brokerEmail: email,
        sections: buildDefaultSections(config.kind),
      });
      router.push(`${config.basePath}/${data._id}`);
    } catch {
      setCreating(false);
    }
  };
                    ...
                    <td className='px-5 py-4'>
                      <div className='flex items-center justify-end gap-1'>
                        {isArchived ? (
                          ...
                          <>
                            <Link href={`${config.basePath}/${t._id}`}>
                              <IconBtn title='Edit'>
        ...
        <div className='fixed inset-0 z-50 flex items-center justify-center bg-secondary/40 p-4 backdrop-blur-sm'>
          <div className='w-full max-w-sm border border-border bg-card p-6 shadow-xl'>
            <h3 className='mb-2 text-lg font-bold text-secondary'>
              Archive {config.docNoun}?
            </h3>
            <p className='mb-6 text-sm text-muted-foreground'>
              This {config.docNoun} will be moved to{' '}
        ...
        <div className='fixed inset-0 z-50 flex items-center justify-center bg-secondary/40 p-4 backdrop-blur-sm'>
          <div className='w-full max-w-sm border border-border bg-card p-6 shadow-xl'>
            ...
            <p className='mb-6 text-sm text-muted-foreground'>
              ...
              this {config.docNoun} from the database. This action cannot be
              undone.
            </p>
            <div className='flex justify-end gap-3'>
