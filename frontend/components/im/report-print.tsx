...
import { ImDocument } from './im-document';
...
/**
 * Broker-only printable view, shared by both report products. It renders OUTSIDE
 * the dashboard chrome (so the whole document paginates like the web page) but
 * inside the admin auth group. `kind` selects the backend service and terminology.
 */
export function ReportPrint({ id, kind }: { id: string; kind: ReportKind }) {
  ...
  useEffect(() => {
    ...
    apiClient
      .get(`${cfg.apiBase}/${id}`, { headers: { Authorization: `Bearer ${token}` } })
      .then(({ data }) => setTemplate(data))
      .catch(() => setError(`Failed to load this ${cfg.docNoun}.`));
  }, [id, token, router, cfg.apiBase, cfg.docNoun]);
      ...
      <div className="sticky top-0 z-10 border-b border-border bg-card/95 backdrop-blur print:hidden">
        <div className="mx-auto flex max-w-4xl items-center justify-between gap-3 px-4 py-3">
          <p className="truncate text-sm text-muted-foreground">
            {template.businessName || cfg.docTitle}
          </p>
          <Button
